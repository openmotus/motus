 ...
 * Validator Library
 *
 * Comprehensive validation for departments, agents, workflows, and related
 * data. Provides naming conventions (kebab-case enforcement), agent type
 * detection via keyword scoring, content validation (descriptions, URLs,
 * schedules, env vars), context schema validation, and name suggestions.
 *
 * @example
 * const validator = new Validator();
 * const nameResult = validator.validateAgentName('weather-fetcher');
 * const typeResult = validator.detectAgentType('Fetches weather data from API');
 */
...
class Validator {
  ...
  /**
   * Validate a department name against naming conventions.
   *
   * @param {string} name - Candidate department name.
  ...
  /**
   * Validate an agent name against naming conventions (kebab-case, action-noun).
   *
   * @param {string} name - Candidate agent name.
  ...
  /**
   * Validate a workflow name against naming conventions.
   *
   * @param {string} name - Candidate workflow name.
   * @returns {{valid: boolean, errors: string[]}}
   */
  ...
  /**
   * Validate an environment variable name (UPPER_SNAKE_CASE).
   *
  ...
  validateEnvVarName(name) {
    ...
    if (typeof name !== 'string') {
      errors.push('Environment variable name must be a string');
      return { valid: false, errors };
    }
  ...
  /**
   * Detect the most likely agent type from a description using keyword scoring.
   *
   * @param {string} description - Free-text agent description.
   * @returns {{type: string, confidence: number, scores: Object}|null} Detection result, or `null` if no keywords matched.
   */
  ...
  /**
   * Detect whether a workflow step should use parallel execution.
   *
   * @param {string} stepDescription - Free-text step description.
   * @returns {{shouldBeParallel: boolean, actionCount: number, confidence: number}}
   */
  detectParallelExecution(stepDescription) {
    if (!stepDescription || typeof stepDescription !== 'string') {
      return { shouldBeParallel: false, actionCount: 0, confidence: 0 };
    }
  ...
  /**
   * Validate a description for length and quality (rejects placeholder text).
   *
   * @param {string} description - Description text to validate.
   * @param {number} [minLength=10] - Minimum character count.
   * @param {number} [maxLength=500] - Maximum character count.
   * @returns {{valid: boolean, errors: string[]}}
   */
  ...
  /**
   * Validate a URL format (must be http or https).
   *
  ...
  /**
   * Validate a schedule format string (e.g. `daily 9:00`, `every 4 hours`).
   *
  ...
  validateSchedule(schedule) {
    ...
    // Validate time ranges (HH:MM where HH=0-23, MM=0-59)
    if (isValid) {
      ...
      if (timeMatch) {
        ...
        if (hours > 23) {
          errors.push(`Invalid hour: ${hours}. Hours must be 0-23.`);
        }
        if (minutes > 59) {
          errors.push(`Invalid minutes: ${minutes}. Minutes must be 0-59.`);
        }
      }
    }
  ...
  /**
   * Validate that a file or directory exists on disk.
   *
  ...
  /**
   * Validate an agent creation context against schema rules.
   *
   * @param {Object} context - Agent context with `name`, `description`, `department`, `type`, etc.
   * @returns {{valid: boolean, errors: string[]}}
   */
  ...
  /**
   * Validate a department creation context against schema rules.
   *
   * @param {Object} context - Department context with `name`, `displayName`, `description`, etc.
   * @returns {{valid: boolean, errors: string[]}}
   */
  ...
  /**
   * Validate a workflow creation context against schema rules.
   *
   * @param {Object} context - Workflow context with `name`, `displayName`, `description`, `department`, `steps`, etc.
   * @returns {{valid: boolean, errors: string[]}}
   */
  ...
  /**
   * Suggest corrected or improved agent names (kebab-case, action-noun).
   *
   * @param {string} name - Original agent name.
   * @returns {string[]} Up to 3 suggested names.
   */
  ...
  /**
   * Suggest appropriate Claude Code tools for an agent type.
   *
   * @param {string} agentType - One of `data-fetcher`, `orchestrator`, `specialist`.
   * @param {boolean} [needsApi=false] - Whether the agent calls external APIs.
   * @returns {string[]} Suggested tool names.
   */
  suggestTools(agentType, needsApi = false) {
    ...
    // Copy to avoid mutating the template arrays on repeated calls
  ...
  /**
   * Generate a suggested environment variable name from department and service.
   *
   * @param {string} department - Department name.
   * @param {string} service - Service/integration name.
   * @returns {string} Suggested env var name in UPPER_SNAKE_CASE.
   */
