 ...
 *
 * Generates all Motus documentation from registry data:
 * - `COMMANDS_REFERENCE.md` — master command list with department summaries
 * - Per-department `.md` docs — agents, workflows, integrations
 * - CLAUDE.md statistics (marker-based update via `<!-- stats:start/end -->` blocks)
 *
 * @example
 * const generator = new DocGenerator();
 * await generator.generate(); // generates all docs to org-docs/
 */
...
class DocGenerator {
  /**
   * Create a DocGenerator.
   *
   * @param {string} [basePath] - Project root directory. Defaults to the
   *   parent of `lib/` (i.e. the Motus repo root).
   */
  constructor(basePath) {
    this.basePath = basePath || path.join(__dirname, '..');
    this.registry = new RegistryManager(this.basePath);
    this.docsPath = path.join(this.basePath, 'org-docs');
    this.deptDocsPath = path.join(this.docsPath, 'departments');
  }

  /**
   * Generate all documentation: commands reference, per-department docs,
   * and CLAUDE.md statistics.
   *
  ...
  /**
   * Generate `COMMANDS_REFERENCE.md` with system overview, department
   * summaries, and available workflows.
   *
   * @returns {Promise<void>}
   */
...
`;
  ...
  /**
   * Generate per-department documentation files into `org-docs/departments/`.
   *
   * @returns {Promise<void>}
   */
...
`;
  ...
  /**
   * Generate markdown documentation for a single integration.
   *
   * @param {Object} integration - Integration config with `name`, `type`, `envVars`, etc.
   * @param {string} departmentName - Owning department name.
   * @returns {string} Markdown content for the integration section.
   */
  generateIntegrationDocs(integration, departmentName) {
    ...
    if (integration.type === 'oauth2') {
      ...
      if (envVars.length > 0) {
        ...
        content += `   echo $${envVars[0]}\n`;
        content += `   \`\`\`\n\n`;
      }
    }

    const allEnvVars = integration.envVars || [];
    ...
    content += `- Make sure .env file is in the project root (same directory as package.json)\n`;
    content += `- Restart any running services after updating .env\n`;
    if (allEnvVars.length > 0) {
      content += `- Check that environment variables are loaded: \`node -e "require('dotenv').config(); console.log(process.env.${allEnvVars[0]})"\`\n`;
    }
    if (integration.type === 'oauth2') {
  ...
  /**
   * Update the statistics block in CLAUDE.md if one is present.
   * Looks for a fenced section between `<!-- stats:start -->` and
   * `<!-- stats:end -->` markers. When the markers are absent, logs
   * current counts and skips the write (no-op for repos that don't
   * use the markers).
   *
   * @returns {Promise<boolean>} `true` if CLAUDE.md was updated, `false` otherwise.
   */
  async updateClaudeMd() {
    ...
    try {
      ...
      if (content.includes(startMarker) && content.includes(endMarker)) {
        const statsBlock = [
          startMarker,
          `- **Departments**: ${stats.departments.total} (${stats.departments.active} active)`,
          `- **Agents**: ${stats.agents.total} (${stats.agents.byType['data-fetcher']} data-fetchers, ${stats.agents.byType['orchestrator']} orchestrators, ${stats.agents.byType['specialist']} specialists)`,
          `- **Workflows**: ${stats.workflows.total} (${stats.workflows.byType['manual'] || 0} manual, ${stats.workflows.byType['scheduled'] || 0} scheduled)`,
          `- **Integrations**: ${stats.integrations.total}`,
        ...
        console.log('  ✓ Updated CLAUDE.md statistics block');
        return true;
      }

      // No markers — log stats but don't modify the file
      console.log(`  ℹ  CLAUDE.md statistics: ${stats.departments.total} departments, ${stats.agents.total} agents, ${stats.workflows.total} workflows`);
      console.log(`  ℹ  Add <!-- stats:start --> and <!-- stats:end --> markers to enable auto-update`);
      return false;
    } catch (error) {
      if (error.code === 'ENOENT') {
        console.log('  ℹ  No CLAUDE.md found — skipping statistics update');
        return false;
      }
      throw error;
    }
