...
class RegistryManager {
  ...
  async addDepartment(data) {
    ...
    const nameValidation = this.validator.validateDepartmentName(name);
    if (!nameValidation.valid) {
      throw new Error(`Invalid department name '${name}': ${nameValidation.errors.join('; ')}`);
    }
  ...
  async addAgent(data) {
    ...
    // Validate name format
    const agentNameValidation = this.validator.validateAgentName(name);
    if (!agentNameValidation.valid) {
      throw new Error(`Invalid agent name '${name}': ${agentNameValidation.errors.join('; ')}`);
    }
  ...
  async addWorkflow(data) {
    ...
    // Validate name format
    const wfNameValidation = this.validator.validateWorkflowName(name);
    if (!wfNameValidation.valid) {
      throw new Error(`Invalid workflow name '${name}': ${wfNameValidation.errors.join('; ')}`);
    }
    ...
    // Update agent usage tracking (store workflow ID for uniqueness across departments)
    if (agents && Array.isArray(agents) && agents.length > 0) {
      const missingAgents = [];
      agents.forEach(agentName => {
        if (this.agents.agents[agentName]) {
          if (!this.agents.agents[agentName].usedInWorkflows.includes(workflowId)) {
            this.agents.agents[agentName].usedInWorkflows.push(workflowId);
          }
        } else {
      ...
      if (missingAgents.length > 0) {
        console.warn(`⚠️  Workflow '${name}' references agents not yet in registry: ${missingAgents.join(', ')}. Register them with addAgent() to enable usage tracking.`);
      }
    }
  ...
  async listWorkflows(filters = {}) {
    ...
    if (filters.type) {
      workflows = workflows.filter(w => w.trigger && w.trigger.type === filters.type);
    }
  ...
  /**
   * Record a workflow execution, updating its run statistics.
   *
   * @param {string} department - Department identifier.
   * @param {string} name - Workflow identifier.
   * @param {Object} [result={}] - Execution result details.
   * @param {boolean} [result.success=true] - Whether the run succeeded.
   * @param {number} [result.durationMs] - Execution time in milliseconds.
   * @param {string} [result.error] - Error message if the run failed.
   * @returns {Promise<Object>} The updated workflow record.
   * @throws {Error} If the workflow does not exist.
   */
  async recordWorkflowRun(department, name, result = {}) {
    await this.ensureLoaded();
    ...
    const success = result.success !== false; // default true
    ...
    const prevRate = workflow.successRate !== undefined ? workflow.successRate : 1;
    const prevSuccesses = Math.round(prevRate * (workflow.runCount - 1));
    workflow.successRate = (prevSuccesses + (success ? 1 : 0)) / workflow.runCount;

    // Store duration if provided
    if (result.durationMs !== undefined) {
      workflow.lastDurationMs = result.durationMs;
    }

    // Store last error if the run failed
  ...
  /**
   * Remove a department and optionally cascade-delete its agents and workflows.
   *
   * @param {string} name - Department identifier.
   * @param {Object} [options={}] - Removal options.
   * @param {boolean} [options.cascade=true] - If `true`, also remove all agents
   *   and workflows belonging to this department. If `false`, throws an error
   *   when the department still has agents or workflows.
   * @returns {Promise<{department: Object, removedAgents: string[], removedWorkflows: string[]}>}
   *   The removed department record and lists of cascade-removed items.
   * @throws {Error} If the department does not exist, or if `cascade` is false
   *   and the department still has agents or workflows.
   */
  async removeDepartment(name, options = {}) {
    await this.ensureLoaded();
    ...
    const department = this.departments.departments[name];
    if (!department) {
      throw new Error(`Department '${name}' not found. Available departments: ${Object.keys(this.departments.departments).join(', ') || '(none)'}`);
    }

    const cascade = options.cascade !== false; // default true

    // Find all agents and workflows belonging to this department
    const deptAgents = Object.values(this.agents.agents)
      .filter(a => a.department === name)
      .map(a => a.name);
    const deptWorkflows = Object.entries(this.workflows.workflows)
      .filter(([, w]) => w.department === name)
      .map(([id]) => id);

    if (!cascade && (deptAgents.length > 0 || deptWorkflows.length > 0)) {
      throw new Error(
        `Cannot remove department '${name}' — it still has ${deptAgents.length} agent(s) and ${deptWorkflows.length} workflow(s). ` +
        `Pass { cascade: true } to remove them, or remove them individually first.`
      );
    }
    ...
    // Cascade-remove workflows first (they reference agents)
    for (const workflowId of deptWorkflows) {
      const workflow = this.workflows.workflows[workflowId];
      // Clean up agent usedInWorkflows references
      if (workflow.agents && Array.isArray(workflow.agents)) {
        workflow.agents.forEach(agentName => {
          const agent = this.agents.agents[agentName];
          if (agent && Array.isArray(agent.usedInWorkflows)) {
            agent.usedInWorkflows = agent.usedInWorkflows.filter(id => id !== workflowId);
          }
        });
      }
      removedWorkflows.push(workflow.name);
      delete this.workflows.workflows[workflowId];
    }

    // Cascade-remove agents
    for (const agentName of deptAgents) {
      removedAgents.push(agentName);
      delete this.agents.agents[agentName];
    }

    // Remove the department itself
    const removedDept = { ...department };
    delete this.departments.departments[name];
  ...
  /**
   * Remove an agent from the registry. Cleans up references in the owning
   * department's agent list and in any workflows that use this agent.
   *
   * @param {string} name - Agent identifier.
   * @returns {Promise<{agent: Object, updatedWorkflows: string[]}>}
   *   The removed agent record and list of workflows that were updated.
  ...
  async removeAgent(name) {
    ...
    const agent = this.agents.agents[name];
    if (!agent) {
      throw new Error(`Agent '${name}' not found. Available agents: ${Object.keys(this.agents.agents).join(', ') || '(none)'}`);
    }

    const updatedWorkflows = [];

    // Remove agent from its department's agent list
    const dept = this.departments.departments[agent.department];
    if (dept && Array.isArray(dept.agents)) {
      dept.agents = dept.agents.filter(a => a !== name);
    }

    // Remove agent from any workflow agent lists
    Object.entries(this.workflows.workflows).forEach(([workflowId, workflow]) => {
      if (Array.isArray(workflow.agents) && workflow.agents.includes(name)) {
        workflow.agents = workflow.agents.filter(a => a !== name);
        updatedWorkflows.push(workflow.name);
      }
    });

    // Remove the agent
    const removedAgent = { ...agent };
    delete this.agents.agents[name];
  ...
  /**
   * Remove a workflow from the registry. Cleans up references in the owning
   * department's workflow list and in agents' `usedInWorkflows` arrays.
   *
   * @param {string} department - Department identifier.
   * @param {string} name - Workflow identifier.
   * @returns {Promise<{workflow: Object, updatedAgents: string[]}>}
   *   The removed workflow record and list of agents that were updated.
   * @throws {Error} If the workflow does not exist.
   */
  async removeWorkflow(department, name) {
    await this.ensureLoaded();
    ...
    // Remove workflow from department's workflow list
    const dept = this.departments.departments[department];
    if (dept && Array.isArray(dept.workflows)) {
      dept.workflows = dept.workflows.filter(w => w !== name);
    }
    ...
    if (workflow.agents && Array.isArray(workflow.agents)) {
      workflow.agents.forEach(agentName => {
        const agent = this.agents.agents[agentName];
        if (agent && Array.isArray(agent.usedInWorkflows)) {
          const before = agent.usedInWorkflows.length;
          agent.usedInWorkflows = agent.usedInWorkflows.filter(id => id !== workflowId);
          if (agent.usedInWorkflows.length < before) {
  ...
  /**
   * Get a comprehensive summary of a department including its agents,
   * workflows, and integration counts in a single call.
   *
   * @param {string} name - Department identifier.
   * @returns {Promise<{department: Object, agents: Object[], workflows: Object[], agentsByType: Object, workflowsByTrigger: Object, integrationCount: number}|null>}
   *   Summary object, or `null` if the department does not exist.
   */
  async getDepartmentSummary(name) {
    await this.ensureLoaded();
    ...
    return {
      ...
      workflowsByTrigger: workflows.reduce((acc, w) => {
        const type = (w.trigger && w.trigger.type) || 'unknown';
        acc[type] = (acc[type] || 0) + 1;
        return acc;
      }, {}),
      integrationCount: department.integrations ? department.integrations.length : 0
    };
  }
  ...
  async validate() {
    ...
    // Check for orphan agents — agents in the registry whose department doesn't list them
    Object.values(this.agents.agents).forEach(agent => {
      const dept = this.departments.departments[agent.department];
      if (dept && !dept.agents.includes(agent.name)) {
        errors.push(`Agent '${agent.name}' belongs to department '${agent.department}' but is not in the department's agents list`);
      }
    });

    // Check for orphan workflows — workflows in the registry whose department doesn't list them
    Object.values(this.workflows.workflows).forEach(workflow => {
      const dept = this.departments.departments[workflow.department];
      if (dept && !dept.workflows.includes(workflow.name)) {
        errors.push(`Workflow '${workflow.name}' belongs to department '${workflow.department}' but is not in the department's workflows list`);
      }
    });
    ...
    Object.values(this.agents.agents).forEach(agent => {
      if (!VALID_AGENT_TYPES.includes(agent.type)) {
        errors.push(`Agent '${agent.name}' has invalid type '${agent.type}'. Valid types: ${VALID_AGENT_TYPES.join(', ')}`);
      }
    });

    // Check usedInWorkflows consistency — ensure each reference points to an existing workflow
    Object.values(this.agents.agents).forEach(agent => {
      if (Array.isArray(agent.usedInWorkflows)) {
        agent.usedInWorkflows.forEach(workflowId => {
          if (!this.workflows.workflows[workflowId]) {
            errors.push(`Agent '${agent.name}' has stale usedInWorkflows reference '${workflowId}' — workflow no longer exists`);
          }
        });
      }
    });

    // Check reverse consistency — workflow agents list should match agent usedInWorkflows
    Object.entries(this.workflows.workflows).forEach(([workflowId, workflow]) => {
      if (Array.isArray(workflow.agents)) {
        workflow.agents.forEach(agentName => {
          const agent = this.agents.agents[agentName];
          if (agent && Array.isArray(agent.usedInWorkflows) && !agent.usedInWorkflows.includes(workflowId)) {
            errors.push(`Workflow '${workflow.name}' lists agent '${agentName}' but agent's usedInWorkflows does not include '${workflowId}'`);
          }
        });
      }
    });
  ...
  async search(query) {
    ...
    if (query === null || query === undefined || typeof query !== 'string' || query.trim() === '') {
      return { departments: [], agents: [], workflows: [] };
    }
  ...
  /**
   * Analyze workflow health across all or filtered workflows. Returns per-workflow
   * health status based on run statistics (success rate, recency, run count).
   *
   * Health grades:
   * - `healthy`  — successRate >= 0.9 and has run recently
   * - `degraded` — successRate between 0.5 and 0.9, or no runs in 7+ days
   * - `failing`  — successRate < 0.5
   * - `idle`     — never been run (runCount === 0)
   *
   * @param {Object} [filters={}] - Optional filter criteria.
   * @param {string} [filters.department] - Only include workflows from this department.
   * @param {string} [filters.status] - Only include workflows with this health status.
   * @returns {Promise<{summary: {total: number, healthy: number, degraded: number, failing: number, idle: number}, workflows: Object[]}>}
   */
  async getWorkflowHealth(filters = {}) {
    await this.ensureLoaded();
    ...
    const analyzed = workflows.map(w => {
      const runCount = w.runCount || 0;
      const successRate = w.successRate !== undefined ? w.successRate : 1;
      const lastRun = w.lastRun ? new Date(w.lastRun).getTime() : null;
      const daysSinceLastRun = lastRun ? (now - lastRun) / (24 * 60 * 60 * 1000) : null;
      ...
      if (runCount === 0) {
        ...
        status = 'failing';
      } else if (successRate < 0.9 || (daysSinceLastRun !== null && daysSinceLastRun > 7)) {
        status = 'degraded';
      } else {
        status = 'healthy';
      }
      ...
      return {
        ...
        daysSinceLastRun: daysSinceLastRun !== null ? Math.round(daysSinceLastRun * 10) / 10 : null,
        lastDurationMs: w.lastDurationMs || null,
        lastError: w.lastError || null
      };
    });

    // Apply status filter after analysis
    const filtered = filters.status
      ? analyzed.filter(w => w.status === filters.status)
      : analyzed;

    const summary = {
      total: filtered.length,
      healthy: filtered.filter(w => w.status === 'healthy').length,
      degraded: filtered.filter(w => w.status === 'degraded').length,
      failing: filtered.filter(w => w.status === 'failing').length,
      idle: filtered.filter(w => w.status === 'idle').length
    };

    return { summary, workflows: filtered };
  }

  /**
   * Analyze agent usage across all workflows. Returns per-agent metrics and
   * a summary breakdown useful for identifying unused agents (removal
   * candidates), overused agents (refactor candidates), and type/department
   * distribution across the registry.
   *
   * Usage levels:
   * - `unused` — not referenced by any workflow (`workflowCount === 0`)
   * - `low`    — referenced by 1 workflow
   * - `medium` — referenced by 2–4 workflows
   * - `high`   — referenced by 5+ workflows
   *
   * @param {Object} [filters={}] - Optional filter criteria.
   * @param {string} [filters.department] - Only include agents from this department.
   * @param {string} [filters.type] - Only include agents of this type (e.g. `data-fetcher`).
   * @param {('unused'|'low'|'medium'|'high')} [filters.usage] - Only include agents with this usage level.
   * @returns {Promise<{summary: {total: number, unused: number, low: number, medium: number, high: number, byType: Object<string, number>, byDepartment: Object<string, number>}, agents: Object[]}>}
   */
  async getAgentUsage(filters = {}) {
    await this.ensureLoaded();
    ...
    const analyzed = agents.map(agent => {
      // Find all workflows that reference this agent. Source of truth: workflow.agents
      // (usedInWorkflows may drift; prefer the live lookup so unused agents are accurate).
      const workflowsUsingAgent = Object.entries(this.workflows.workflows)
        .filter(([, w]) => Array.isArray(w.agents) && w.agents.includes(agent.name))
        .map(([id, w]) => ({ id, name: w.name, department: w.department }));

      const workflowCount = workflowsUsingAgent.length;
      ...
      return {
        ...
        usage,
        workflowCount,
        workflows: workflowsUsingAgent
      };
    });

    const filtered = filters.usage
      ? analyzed.filter(a => a.usage === filters.usage)
      : analyzed;
    ...
    const summary = {
      total: filtered.length,
      unused: filtered.filter(a => a.usage === 'unused').length,
      low: filtered.filter(a => a.usage === 'low').length,
      medium: filtered.filter(a => a.usage === 'medium').length,
      high: filtered.filter(a => a.usage === 'high').length,
      byType,
      byDepartment
    };
  ...
  async import(data) {
    ...
    // Validate structure of each registry section before overwriting
    if (data.departments !== undefined) {
      if (!data.departments || typeof data.departments !== 'object' ||
          typeof data.departments.departments !== 'object' ||
          typeof data.departments.metadata !== 'object') {
        throw new Error('Invalid departments data: must have { departments: {}, metadata: {} } structure.');
      }
      this.departments = JSON.parse(JSON.stringify(data.departments));
    }
    if (data.agents !== undefined) {
      if (!data.agents || typeof data.agents !== 'object' ||
          typeof data.agents.agents !== 'object' ||
          typeof data.agents.metadata !== 'object') {
        throw new Error('Invalid agents data: must have { agents: {}, metadata: {} } structure.');
      }
    ...
    if (data.workflows !== undefined) {
      if (!data.workflows || typeof data.workflows !== 'object' ||
          typeof data.workflows.workflows !== 'object' ||
          typeof data.workflows.metadata !== 'object') {
        throw new Error('Invalid workflows data: must have { workflows: {}, metadata: {} } structure.');
      }
      this.workflows = JSON.parse(JSON.stringify(data.workflows));
    }
    this.loaded = true;
    await this.save();
  }
  ...
  /**
   * Export the entire registry as a Mermaid flowchart diagram.
   *
   * Renders departments → agents → workflows as a directed graph. Useful for
   * README diagrams, design reviews, and visualizing how agents are wired
   * across workflows.
   *
   * @param {Object} [options]
   * @param {('TD'|'TB'|'BT'|'LR'|'RL')} [options.direction='LR'] - Mermaid layout direction.
   * @param {string} [options.department] - Filter to a single department.
   * @param {boolean} [options.includeWorkflows=true] - Include workflow nodes/edges.
   * @param {boolean} [options.includeAgents=true] - Include agent nodes/edges.
   * @param {boolean} [options.includeOrphans=true] - Include agents/workflows that don't belong to any department.
   * @param {string} [options.title] - Optional diagram title rendered above the flowchart.
   * @returns {Promise<string>} Mermaid flowchart source (multi-line string, no fences).
   */
  async exportMermaid(options = {}) {
    await this.ensureLoaded();
    ...
    const validDirections = ['TD', 'TB', 'BT', 'LR', 'RL'];
    if (!validDirections.includes(direction)) {
      throw new Error(`Invalid direction "${direction}". Must be one of: ${validDirections.join(', ')}.`);
    }

    // Mermaid node IDs must be alphanumeric/underscore. Quote labels to handle spaces/special chars.
    const safeId = (prefix, name) =>
      `${prefix}_${String(name).replace(/[^a-zA-Z0-9]/g, '_')}`;
    ...
    const departments = Object.values(this.departments.departments)
      .filter(d => !deptFilter || d.name === deptFilter);
    const agents = Object.values(this.agents.agents);
    const workflows = Object.values(this.workflows.workflows);

    const departmentNames = new Set(departments.map(d => d.name));

    // Track which agents/workflows are reachable so we can render orphans separately
    const renderedAgents = new Set();
    const renderedWorkflows = new Set();
    const edges = [];

    // Department subgraphs
    for (const dept of departments) {
      const deptId = safeId('dept', dept.name);
      lines.push(`  subgraph ${deptId}[${quoteLabel(`📁 ${dept.displayName || dept.name}`)}]`);

      if (includeAgents) {
        const deptAgents = agents.filter(a => a.department === dept.name);
        for (const agent of deptAgents) {
          const agentId = safeId('agent', agent.name);
          const typeIcon = agent.type === 'data-fetcher' ? '📥'
            : agent.type === 'orchestrator' ? '🎯'
            : agent.type === 'specialist' ? '🛠️' : '🤖';
          lines.push(`    ${agentId}[${quoteLabel(`${typeIcon} ${agent.name}`)}]`);
          renderedAgents.add(agent.name);
        }
      }

      if (includeWorkflows) {
        const deptWorkflows = workflows.filter(w => w.department === dept.name);
        for (const wf of deptWorkflows) {
          const wfId = safeId('wf', `${wf.department}_${wf.name}`);
          lines.push(`    ${wfId}([${quoteLabel(`⚡ ${wf.name}`)}])`);
          renderedWorkflows.add(`${wf.department}/${wf.name}`);

          if (includeAgents && Array.isArray(wf.agents)) {
            for (const agentName of wf.agents) {
              if (renderedAgents.has(agentName)) {
                edges.push(`  ${safeId('agent', agentName)} --> ${wfId}`);
              }
            }
          }
        }
    ...
    // Orphans — agents/workflows whose department isn't in the filtered set
    if (includeOrphans && !deptFilter) {
      const orphanAgents = includeAgents
        ? agents.filter(a => !departmentNames.has(a.department))
        : [];
      const orphanWorkflows = includeWorkflows
        ? workflows.filter(w => !departmentNames.has(w.department))
        : [];

      if (orphanAgents.length > 0 || orphanWorkflows.length > 0) {
        lines.push(`  subgraph orphans["⚠️ Orphans"]`);
        for (const agent of orphanAgents) {
          const agentId = safeId('agent', agent.name);
          lines.push(`    ${agentId}[${quoteLabel(`🤖 ${agent.name}`)}]`);
          renderedAgents.add(agent.name);
        }
