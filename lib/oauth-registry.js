 ...
 *
 * Manages OAuth configurations in the OAuth Manager's `server.js`.
 * Automatically registers new OAuth2 integrations when departments
 * are created with OAuth-based services. Provides code generation
 * for service configs, init functions, connect/callback handlers.
 *
 * @example
 * const registry = new OAuthRegistry('/path/to/motus');
 * await registry.load();
 * const added = await registry.addIntegration({ id: 'slack', name: 'Slack', envVars: ['SLACK_CLIENT_ID', 'SLACK_CLIENT_SECRET'], authUrl: '...', tokenUrl: '...' });
 */
...
class OAuthRegistry {
  ...
  _generateInitFunction(integration) {
    ...
    const envChecks = envVars.slice(0, 2).map(v => `!process.env.${v}`).join(' || ');
    func += `  if (${envChecks}) {\n`;
    func += `    return null;\n`;
    func += `  }\n`;
    func += `  \n`;
    ...
    func += envVars.length > 1 ? `    clientSecret: process.env.${envVars[1]},\n` : '';
